...
```
//輸入Wi-Fi帳密
const char* ssid     = "WIFI NAME";       // 請修改 Wi-Fi 熱點名稱
const char* password = "WIFI PASSWORD";   // 請修改 Wi-Fi 熱點密碼
  ...
  <script>
    ...
    async function predict() {
      ...
      if (maxPredictions>0) {
        ...
        result.innerHTML += "<br><b>Result: " + maxClassName + "," + maxProbability.toFixed(2) + "</b>";

        $.ajax({url: document.location.origin+'/?serial='+maxClassName+";"+maxProbability+';stop',async: false});
        // ===== 語音播報 =====
        const now = Date.now();
        if (now - lastSpeakTime >= 3000) {
            if (maxClassName.toLowerCase() == "a") {
              speakResult("這是剪刀");
            }
            else if (maxClassName.toLowerCase() == "b") {
              speakResult("這是石頭");
            }
            else if (maxClassName.toLowerCase() == "c") {
              speakResult("這是布");
            }
            else if (maxClassName.toLowerCase() == "d") {
              speakResult("這是沒出拳");
            }
            else{
      ...
      setTimeout(function() { getStill.click(); }, 3000);
    }   
  </script>   
)rawliteral";
